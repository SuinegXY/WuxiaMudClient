...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK || event.Type == UIEVENT_RBUTTONDOWN )
		{
			...
			if ( event.Type == UIEVENT_BUTTONDOWN )
			{
				...
				if ( pControl && _tcsicmp(pControl->GetClass(), _T("KeyLayoutUI")) == 0 )
				{
		...
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
				if( ::PtInRect(&m_rcItem, event.ptMouse) ) ActivateR();
				m_uButtonState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
				Invalidate();
			}
			return;
	...
	bool CButtonUI::ActivateR()
	{
		if( !CControlUI::Activate() ) return false;
		if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_MSGTYPE_CLICKR);
		return true;
	}
	...
	{
		if ( m_uButtonState & UISTATE_HOT && m_dwHotColor )
		{
			CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwHotColor));
		}
