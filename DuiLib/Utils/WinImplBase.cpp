...
//         return HTLEFT; 
//     else if(pt.x >= rcClient.right-3)   
//         return HTRIGHT;
//     else if(pt.y <= rcClient.top+3)
//         return HTTOP;
//     else if(pt.y >= rcClient.bottom-3)
//         return HTBOTTOM;
...
//         return HTBOTTOMLEFT;
//     else if(pt.x >= rcClient.right-10 && pt.y >= rcClient.bottom-10)
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	const WCHAR* pcszSkinFile = GetSkinFile().GetData();
	if (GetResourceType()==UILIB_RESOURCE)
	{
		STRINGorID xml(_ttoi(pcszSkinFile));
		pRoot = builder.Create(xml, _T("xml"), this, &m_PaintManager);
	}
	else
	{
		pRoot = builder.Create(pcszSkinFile, (UINT)0, this, &m_PaintManager);
	}
	ASSERT(pRoot);
	if (pRoot==NULL)
	{
...
LRESULT WindowImplBase::OnSetCursor(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    bHandled = FALSE;
    DWORD nHitTest = LOWORD(lParam);
