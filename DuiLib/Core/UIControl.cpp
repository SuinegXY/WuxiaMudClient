...
{
    if( _tcscmp(m_sBkImage, pStrImage) == 0 ) return;

    m_sBkImage = pStrImage;
    Invalidate();
}

void CControlUI::SetBkImage( LPCSTR pStrImage )
{
	wstring	wstrImage = L"";
	WCHAR* pBuff = NULL;
	do 
	{
		int nCount = MultiByteToWideChar(CP_ACP, 0, pStrImage, -1, NULL, 0) * sizeof(WCHAR);
		if ( 0 == nCount ) break;
		pBuff = new WCHAR[nCount+1];
		if ( NULL == pBuff ) break;
		nCount = MultiByteToWideChar(CP_ACP, 0, pStrImage, -1, pBuff, nCount);
		if ( 0 == nCount )break;
		wstrImage = pBuff;
	} while (FALSE);
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("focuscolor")) == 0 ) {
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		if ( clrColor )
	...
	if(m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)
	{
		if(m_nBorderSize > 0 && ( m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0 ))//画圆角边框
		{
			if (IsFocused() && m_dwFocusBorderColor != 0)
				CRenderEngine::DrawRoundRect(hDC, m_rcItem, m_nBorderSize, m_cxyBorderRound.cx, m_cxyBorderRound.cy, GetAdjustColor(m_dwFocusBorderColor), m_bDot);
			else
				CRenderEngine::DrawRoundRect(hDC, m_rcItem, m_nBorderSize, m_cxyBorderRound.cx, m_cxyBorderRound.cy, GetAdjustColor(m_dwBorderColor), m_bDot);
		}
		else
		{
			...
			else if(m_rcBorderSize.left > 0 || m_rcBorderSize.top > 0 || m_rcBorderSize.right > 0 || m_rcBorderSize.bottom > 0)
			{
				...
				CRenderEngine::DrawRect(hDC, m_rcItem, m_nBorderSize, GetAdjustColor(m_dwBorderColor), m_bDot);
		}
	}
	if ( m_bResizeMark )
	{
#define		MARK_DISTINCK	5
#define		MARK_LENGTH		10
#define		MARK_LESS		2

		RECT rcMark = m_rcItem;
		for ( int i = 0; i < 3; i++ )
		{
			rcMark.left = m_rcItem.right - MARK_DISTINCK - i*MARK_LESS;
			rcMark.right= rcMark.left;
			rcMark.bottom = m_rcItem.bottom - MARK_DISTINCK - i*MARK_LESS;
			rcMark.top  = rcMark.bottom - MARK_LENGTH + i*MARK_LESS*2;
			CRenderEngine::DrawLine(hDC, rcMark, 1, GetAdjustColor(m_dwBorderColor), /*m_bDot ? PS_DOT : */m_nBorderStyle);

			RECT rcMark = m_rcItem;
			rcMark.right= m_rcItem.right - MARK_DISTINCK - i*MARK_LESS;
			rcMark.left = rcMark.right - MARK_LENGTH + i*MARK_LESS*2;
			rcMark.bottom = m_rcItem.bottom - MARK_DISTINCK - i*MARK_LESS;
			rcMark.top  = rcMark.bottom;
			CRenderEngine::DrawLine(hDC, rcMark, 1, GetAdjustColor(m_dwBorderColor), /*m_bDot ? PS_DOT : */m_nBorderStyle);
		}
	}
}
...
void CControlUI::SetBorderDot( bool bDot /*= TRUE*/ )
{
	m_bDot = bDot;
}

void CControlUI::SetResizeMark( bool bResizeMark /*= true*/ )
{
	m_bResizeMark = bResizeMark;
}

void CControlUI::SetFloatPos( RECT rc )
{
	SIZE sizeLocation = {GetPos().left - m_cXY.cx, GetPos().top - m_cXY.cy};
	rc.left += sizeLocation.cx;
	rc.right += sizeLocation.cx;
	rc.top += sizeLocation.cy;
	rc.bottom += sizeLocation.cy;
	SetPos(rc);
}
