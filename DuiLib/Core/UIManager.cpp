	...
	case WM_PAINT:
		{
			...
			if( m_bAlphaBackground )
			{
				...
				PAINTSTRUCT ps = { 0 };
				m_hDcPaint = ::BeginPaint(m_hWndPaint, &ps);
				...
				{
					memset( m_pBmpBackgroundBits+(i*width+morrorLeft)*4, 0, (morrorRight-morrorLeft)*sizeof(BYTE)*4 );
    ...
    case WM_PRINTCLIENT:
        {
            ...
            if( (lParam & PRF_CHILDREN) != 0 ) {
                ...
                while( hWndChild != NULL ) {
					...
					HDC hScrDC;
					hScrDC = ::GetDC(hWndChild);
					// 					HDC hCloneDC = ::CreateCompatibleDC(hDC);
					// 					BitBlt(hCloneDC, 0,0,cx, cy,hScrDC, 0, 0, SRCCOPY);
					BitBlt(hDC, 0,0,cx, cy,hScrDC, 0, 0, SRCCOPY);
					//					::DeleteDC(hCloneDC);
					::ReleaseDC(hWndChild,hScrDC);
...
UINT CPaintManagerUI::GetLocalTimerID()
{
	m_uLocalTimerID++;
	return m_uLocalTimerID;
}
