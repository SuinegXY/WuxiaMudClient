...
extern "C"
{
	...
	extern TGifInfo* stbi_load_from_memory_gif(unsigned char const *buffer, int len, int *n, int *comp, int req_comp);
...
CGifHandler* CRenderEngine::LoadGif( STRINGorID bitmap, LPCTSTR type, DWORD mask )
{
	...
	TGifInfo* pGifInfos = (TGifInfo*)stbi_load_from_memory_gif(pData, dwSize, &count, &n, 4);
	delete[] pData;
	pData = NULL;
