...
*/ 
char* CMD5::MD5String(const char *string ) 
{ 
	MD5_CTX context; 
...
*/ 
bool CMD5::MD5Check( char *md5string, char* string ) 
{ 
	return strcmp( md5string, MD5String( string ) ) == 0; 
}

wstring CMD5::Str2Md5(wstring const & Str)
{
	...
	char *poText = MD5String((char*)(LPCTSTR)psText);
	delete []psText;

	dwNum = MultiByteToWideChar (CP_ACP, 0, poText, -1, NULL, 0);
	wchar_t *pwText;
	...
	MultiByteToWideChar (CP_ACP, 0, poText, -1, pwText, dwNum);
	wstring Md5Str = pwText;
	delete []pwText;
	return Md5Str;
}


char* CMD5::MD5String( FILE *pfile) 
{ 
	MD5_CTX context; 
	unsigned char digest[16]; 
	char output1[34]; 
...
bool CMD5::MD5Check(const char *md5string, FILE *file ) 
{ 
	return strcmp( md5string, MD5String( file ) ) == 0; 
}


char* CMD5::MD5String( FILE *pfile,void* pdatabuf,unsigned int len) 
{ 
	MD5_CTX context; 
	unsigned char digest[16]; 
	char output1[34]; 
	...
	fread(pdatabuf,1,len,pfile);
	int i; 

	MD5Init( &context); 
	MD5Update( &context, (unsigned char *)pdatabuf, len ); 
	MD5Final( digest, &context ); 
